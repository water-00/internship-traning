...
class QuestionClassifier:
    ...
    def build_wdtype_dict(self):
        wd_dict = {}
        self.words = set(
            self.person_words + self.type_words + self.piece_words + self.relation_words + self.relation_qwds)
...
class AnswerSearcher:
    ...
    def answer_prettify(self, question_type, answers, question):
        # 未知种类问题
        if question_type == 'unknow':
            final_answer = "抱歉，我不太理解您的意思😥，您可以尝试换一个问题或换一种问法🤔。"
            return final_answer
        ...
        if question_type == 'composer_specific_type_pieces':
            ...
            for data in answers:
                node = data['m']
                composer_dic[node['composer']].append(
                    {'name': node['name'], 'opus': node['opus'], 'type': node['type']})
            final_answer = "您好！关于您给出的问题“{0}”，这是我查阅数据库的返回结果😊！".format(question)
            for key, value in composer_dic.items():
                final_answer += "\n作曲家{0}的作品有：".format(key)
                for piece in value:
                    final_answer += "\n  名称：{0} opus编号：{1} 类型：{2}（{3}）".format(piece['name'], piece['opus'],
                                                                                     piece['type'] if piece[
                                                                                                          'type'] != '' else "other",
                                                                                     self.classifier.type_dic_e2c[
                                                                                         piece['type']])
        ...
        # 查询某人的特定关系人
            ...
            final_answer = "您好！关于您给出的问题“{0}”，我查出了以下相关人物😊，他们和您问题中涉及的人物的关系如下：".format(
                question)
            for data in answers:
                path = data['p']
                final_answer += "\n  " + str(path)
        ...
        # 查询所有跟某人有关系的人
            ...
            final_answer = "您好！关于您给出的问题“{0}”，我查到了以下相关人物😊，他们和您问题中涉及的人物的关系如下：".format(
                question)
            for data in answers:
                path = data['p']
                final_answer += "\n  " + str(path)
            ...
            for data in answers:
                ...
                if 'Composer' in label:
                    ...
                    final_answer += "{n}是一位伟大的作曲家，他于{t}出生于{p}，于{d}逝世。\n{n}的一生创作了很多的曲目，您可以通过搜索如“XXX写过哪些曲子？”这类问题来快速了解他的作品。\n您也可以通过搜索“XXX的人际关系？”这类问题来快速了解他的人际关系😊。\n欢迎继续向我提问😀！".format(
                        n=name, t=birth_time, p=birth_place, d=death_time)
                else:
                    final_answer += "抱歉，我没有查到关于{n}的更多信息😔，您可以搜索“XXX的人际关系？”这类问题来了解他的人际关系。".format(
                        n=name)
